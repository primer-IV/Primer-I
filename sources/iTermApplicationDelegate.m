...
//    }
    ...
    [suUpdater setUpdateCheckInterval:978307200.0];
    [suUpdater setAutomaticallyChecksForUpdates:NO];
    
    //NSTimeInterval myTime;
    //myTime = [suUpdater updateCheckInterval];
...
//    [menu addItem:item];
    ...
    item = [[[NSMenuItem alloc] initWithTitle:@"Quit Primer"
...
- (void)performStartupActivities {
    ...
    // Set font, securing size:
    PTYSession *session = [term currentSession];
    DLog(@"current session: %@", session);
    NSFont * myFont =[NSFont fontWithName:@"CourierNewPSMT" size:24];
    [session setFont:myFont nonAsciiFont:myFont horizontalSpacing:1 verticalSpacing:1] ;
    
    //Copy font to library
    NSError * myErr;
    [[NSFileManager defaultManager] copyItemAtPath:[NSString stringWithFormat:@"%@%@",[[NSBundle mainBundle] resourcePath],@"/Droid Sans Mono for Powerline Nerd Font Complete.otf"] toPath:@"/Library/Fonts/Droid Sans Mono for Powerline Nerd Font Complete.otf" error:&myErr];
